...
async function verify() {
  ...
  console.log("Games after cutoff:", games.length);
  console.log("");
  ...
  games.forEach((game) => {
    ...
    console.log("Processing game:", game.id.slice(0,8));
    ...
    if (standings[0] && standings[0].score > 0 && stats[standings[0].pid]) {
      const winnerName = stats[standings[0].pid].name;
      console.log("  Winner:", winnerName, "with", standings[0].score, "pts");
      stats[standings[0].pid].cmp++;
    } else {
      console.log("  No winner (all 0 pts)");
    }
    ...
    if (state.circuitResults) {
      state.circuitResults.forEach((cr, idx) => {
        ...
        if (winner && winner[1] > 0 && stats[winner[0]]) {
          console.log("  Circuit", idx+1, "winner:", stats[winner[0]].name, "with", winner[1], "pts");
          stats[winner[0]].vic++;
        }
      });
    }
  ...
  console.log("=== EXPECTED HALL OF FAME STATS (WITH CUTOFF) ===");
